    ...
    <div className="space-y-5">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <button onClick={addSKU} className="text-[10px] text-blue-600 hover:text-blue-800">+ SKU追加</button>
        </div>
        <div className="bg-gray-50 rounded-lg border border-gray-200 overflow-x-auto">
          <table className="w-full text-[11px]">
            <thead>
              <tr className="border-b border-gray-200">
                <th className="text-left px-3 py-2 text-gray-500 font-medium">SKU名</th>
                <th className="text-right px-3 py-2 text-gray-500 font-medium">単価(税抜)</th>
                <th className="text-right px-3 py-2 text-gray-500 font-medium whitespace-nowrap">商品数/取引</th>
                <th className="text-right px-3 py-2 text-gray-500 font-medium whitespace-nowrap">取引回数/人</th>
                <th className="text-right px-3 py-2 text-gray-500 font-medium whitespace-nowrap">年間購入回数</th>
                <th className="text-right px-3 py-2 text-blue-600 font-semibold whitespace-nowrap">一人あたり売上</th>
                <th className="w-8"></th>
              </tr>
            </thead>
            <tbody>
              {config.skus.map(function(sku) {
                ...
                return (
                  <tr key={sku.id} className="border-b border-gray-100 last:border-0 hover:bg-white">
                    ...
                    <td className="px-3 py-2 text-right">
                      <span className="font-mono font-semibold text-blue-700">{formatYen(revPerPerson)}円</span>
                    </td>
      ...
      <div>
        <h4 className="text-xs font-semibold text-gray-700 mb-2">UNIT ECONOMICS</h4>
        <div className="bg-blue-50 rounded-lg border border-blue-100 overflow-hidden">
          <div className="grid grid-cols-5 divide-x divide-blue-100">
            ...
            <div className="px-3 py-3 text-center">
              <div className="text-[9px] text-gray-500 mb-1">CAC</div>
              <div className="mb-1">
                <NumInput value={config.cac}
                  onChange={function(v) { onChange({ ...config, cac: v }) }}
                  className="text-center font-semibold text-gray-900" suffix="円" />
              </div>
              <div className="text-[9px] text-gray-400">顧客獲得コスト</div>
            </div>

            {/* LTV */}
            <div className="px-3 py-3 text-center">
              <div className="text-[9px] text-gray-500 mb-1">LTV</div>
              <div className="text-sm font-mono font-bold text-blue-700 mb-1">{formatYen(ltv)}円</div>
              <div className="text-[9px] text-gray-400">生涯顧客価値</div>
            </div>

            {/* LTV/CAC */}
            <div className="px-3 py-3 text-center">
              <div className="text-[9px] text-gray-500 mb-1">LTV/CAC</div>
            ...
            {/* Payback */}
            <div className="px-3 py-3 text-center">
              <div className="text-[9px] text-gray-500 mb-1">回収期間</div>
              <div className={'text-sm font-mono font-bold mb-1 '
                + (payback <= 12 ? 'text-green-600' : payback <= 24 ? 'text-amber-600' : 'text-red-600')}>
                {payback.toFixed(0)}ヶ月
              </div>
              <div className="text-[9px] text-gray-400">CAC回収</div>
            </div>

            {/* Monthly Churn */}
            <div className="px-3 py-3 text-center">
              <div className="text-[9px] text-gray-500 mb-1">月次解約率</div>
              <div className="mb-1">
              ...
              <div className="text-[9px] text-gray-400">平均{avgLifeMonths.toFixed(0)}ヶ月</div>
            </div>
          </div>
        </div>

        {/* Reference bars */}
        <div className="mt-2 flex gap-2 text-[9px] text-gray-400">
...
function NumInput({ value, onChange, suffix, step, min, max, className }: {
  ...
  if (editing) {
    return (
      <input type="text" value={draft}
        ...
        autoFocus
        className="w-20 text-right bg-white border border-blue-300 rounded px-1 py-0.5 text-[11px] font-mono outline-none focus:ring-1 focus:ring-blue-400"
      />
    )
  }
  ...
  return (
    <button onClick={startEdit}
      className={'font-mono text-[11px] px-1 py-0.5 rounded border border-transparent hover:border-blue-300 hover:bg-blue-50 cursor-pointer transition-colors ' + (className || 'text-gray-900')}>
