    ...
    <div className="space-y-5">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <h4 className="text-xs font-semibold text-sand-600">職種カテゴリ定義</h4>
          ...
          <table className="w-full text-[11px]">
            <thead>
              <tr className="border-b border-cream-200">
                <th className="text-left px-3 py-2 text-sand-500 font-medium">職種名</th>
                <th className="text-right px-3 py-2 text-sand-500 font-medium">月額単価(請求)</th>
                <th className="text-right px-3 py-2 text-sand-500 font-medium">月額原価</th>
            ...
            <tbody>
              {config.categories.map(function(cat) {
                ...
                return (
                  <tr key={cat.id} className="border-b border-cream-200 last:border-0 hover:bg-white">
                    ...
                    <td className="px-1 py-2">
                      {config.categories.length > 1 && (
                        <button onClick={function() { removeCategory(cat.id) }} className="text-sand-300 hover:text-red-500 transition-colors">
      ...
      <div className="flex items-center gap-3 text-[11px] px-1">
        <span className="text-sand-500 font-medium">稼働率:</span>
      ...
      <div>
        <h4 className="text-xs font-semibold text-sand-600 mb-2">稼働人数推移</h4>
        <div className="bg-amber-50 rounded-lg border border-amber-100 overflow-x-auto">
          <table className="w-full text-[11px]">
            <thead>
              <tr className="border-b border-amber-200">
                <th className="text-left px-3 py-2 text-sand-500 font-medium">職種</th>
            ...
            <tbody>
              {config.categories.map(function(cat) {
                return (
                  <tr key={cat.id} className="border-b border-amber-100 last:border-0 hover:bg-white/50">
                    <td className="px-3 py-2 font-medium text-sand-600">{cat.name}</td>
              ...
              <tr className="border-t border-amber-200 bg-amber-100/30">
                <td className="px-3 py-2 text-sand-500">売上</td>
                {fyRevenue.map(function(rev, fi) {
                  return <td key={fi} className="px-3 py-2 text-right font-mono text-sand-600">{formatYen(rev)}円</td>
                })}
              </tr>
              <tr className="border-t border-amber-200 bg-amber-100/30">
                <td className="px-3 py-2 text-sand-500">原価</td>
                {fyCost.map(function(cost, fi) {
                  return <td key={fi} className="px-3 py-2 text-right font-mono text-sand-500">{formatYen(cost)}円</td>
                })}
              </tr>
                ...
                {fyRevenue.map(function(rev, fi) {
                  return (
                    <td key={fi} className="px-3 py-2 text-right">
                      ...
                      <span className="text-[10px] text-sand-400 ml-1">({(fyMargin[fi] * 100).toFixed(0)}%)</span>
                    </td>
                  )
                })}
              </tr>
