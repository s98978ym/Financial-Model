    ...
    <div className="space-y-5">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <h4 className="text-xs font-semibold text-sand-600">資産カテゴリ定義</h4>
          ...
          <table className="w-full text-[11px]">
            <thead>
              <tr className="border-b border-cream-200">
                <th className="text-left px-3 py-2 text-sand-500 font-medium">資産名</th>
                <th className="text-right px-3 py-2 text-sand-500 font-medium">月額料金</th>
                <th className="text-right px-3 py-2 text-sand-500 font-medium">取得原価</th>
            ...
            <tbody>
              {config.assets.map(function(asset) {
                ...
                return (
                  <tr key={asset.id} className="border-b border-cream-200 last:border-0 hover:bg-white">
                    <td className="px-3 py-2">
                      <input type="text" value={asset.name}
                        onChange={function(e) { updateAsset(asset.id, { name: e.target.value }) }}
                        className="w-full bg-transparent font-medium text-dark-900 outline-none focus:bg-cream-50 focus:rounded px-1 -mx-1" />
                    </td>
                    ...
                    <td className="px-1 py-2">
                      {config.assets.length > 1 && (
                        <button onClick={function() { removeAsset(asset.id) }} className="text-sand-300 hover:text-red-500 transition-colors">
      ...
      <div className="flex items-center gap-6 text-[11px] px-1">
        ...
        <div className="flex items-center gap-2">
          <span className="text-sand-500 font-medium">平均契約期間:</span>
          <CellInput value={config.avg_contract_months}
            onChange={function(v) { onChange({ ...config, avg_contract_months: v }) }} integer />
          <span className="text-sand-400">ヶ月</span>
        </div>
      </div>

      {/* Unit count projections */}
      <div>
        <h4 className="text-xs font-semibold text-sand-600 mb-2">保有台数推移</h4>
        <div className="bg-stone-50 rounded-lg border border-stone-200 overflow-x-auto">
          <table className="w-full text-[11px]">
            <thead>
              <tr className="border-b border-stone-300">
                <th className="text-left px-3 py-2 text-sand-500 font-medium">資産</th>
              ...
              {config.assets.map(function(asset) {
                return (
                  <tr key={asset.id} className="border-b border-stone-100 last:border-0 hover:bg-white/50">
                    <td className="px-3 py-2 font-medium text-sand-600">{asset.name}</td>
