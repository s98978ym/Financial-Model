...
export default function Phase3Page() {
  ...
  const { result, isProcessing, isComplete, isFailed, trigger, progress, error, projectState, status } =
  ...
  const log = (msg: string) => {
    ...
    setClickLog((prev) => [...prev.slice(-4), `${ts} ${msg}`])
    console.log(`[Phase3] ${msg}`)
  }

  const handleTrigger = () => {
    log(`クリック → status=${status}, projectId=${projectId}`)
    log(`API=${apiUrl}, phase2Proposal=${phase2Proposal ? 'あり' : 'なし'}`)
    try {
      trigger({ selected_proposal: phase2Proposal || {} })
      log('trigger() 呼び出し完了')
    } catch (e: any) {
      log(`trigger() 例外: ${e.message}`)
    }
  }
    ...
    >
      ...
      <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-xs font-mono">
        <div className="flex flex-wrap gap-x-4 gap-y-1">
          ...
          <span>projectId: {projectId?.slice(0, 8) || 'なし'}</span>
          <span>API: {apiUrl}</span>
          <span>phase2: {phase2Proposal ? 'loaded' : 'なし'}</span>
          <span>phases: {projectState?.phase_results ? Object.keys(projectState.phase_results).join(',') : 'none'}</span>
        </div>
        {clickLog.length > 0 && (
          <div className="mt-2 border-t border-yellow-200 pt-2 space-y-0.5">
            {clickLog.map((l, i) => <div key={i}>{l}</div>)}
          </div>
        )}
